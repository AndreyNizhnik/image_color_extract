...
def get_img_colors():
    ...
    for code in color_codes:
        if convert_rgb_to_names(code) not in color_names and len(color_names) < NUMBER_OF_COLORS:
            ...
            colors_text.insert(END, f"{i}. {convert_rgb_to_names(code)}   ({rgb_to_hex(code)}) {code}\n")
            colors_text.tag_add(f"{i}", f"{i}.0", f"{i}.50")
            colors_text.tag_config(f"{i}", foreground=convert_rgb_to_names(code))
            i += 1
...
root = Tk()
root.title('Image Color Extractor v1.0.12')
root.config(padx=25, pady=25, bg=BACKGROUND_COLOR)
title_label = Label(text="Image Color Extract", font=(TITLE_FONT, 31, "bold"), fg=TITLE_COLOR, bg=BACKGROUND_COLOR)
